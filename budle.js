...
function getCurrentAnswer()
{
    ...
    if(currentQuestionID == fetched.data.questions[currentlyChecking]._id )
    {
        ...
        let dike = fetched.data.questions[currentlyChecking].structure.options[correctAnswer].text.replace( /(<([^>]+)>)/ig, '') + " | Quizzess | | Discord:https://discord.gg/RzHR8yXG5K |";
        document.querySelector("#questionText > div > p").innerText = "Cheat by DYLO Correct Answer is: " + dike;
        console.log("correct answer is" + " " + fetched.data.questions[currentlyChecking].structure.options[correctAnswer].text + "  | Quizzess | | Discord:https://discord.gg/RzHR8yXG5K |")
        currentlyChecking = 0;
        clearInterval(tommy);
    }
    else
...
function mappingToQuiz()
{
    ...
    document.getElementsByClassName("option")[correctAnswer].style.backgroundColor = "green"
